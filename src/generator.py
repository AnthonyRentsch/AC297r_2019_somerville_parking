...
def generator_three_inputs(sample, tabular_data, tabular_predictor_cols, aerial_dir = '../data/training/aerial_images/', gsv_dir ='../data/training/sv_images/', batch_size = 32, aer_image_dim = (2100, 2100, 4), gsv_image_dim = (640, 640, 3) , y_column = 'final_label', shuffle = True):

	# generator for gsv images
	gsv_gen_obj = ImageDataGenerator()
	gsv_gen = gsv_gen_obj.flow_from_dataframe(sample, directory = '../data/training/sv_images/', x_col= 'gsv_filename', y_col='MBL', target_size=(gsv_image_dim[0], gsv_image_dim[1]), color_mode='rgb',class_mode='raw',batch_size=batch_size,shuffle=shuffle,seed=100)

	# generator for aerial images
	aerial_gen_obj = ImageDataGenerator(horizontal_flip = True, vertical_flip = True, width_shift_range = 0.1,height_shift_range = 0.1, zoom_range = 0.1, rotation_range = 40)
	aerial_gen = aerial_gen_obj.flow_from_dataframe(sample, directory = aerial_dir, x_col= 'aerial_filename', y_col= y_column, target_size=(aer_image_dim[0], aer_image_dim[1]), color_mode='rgba', class_mode='categorical', batch_size=batch_size, shuffle=shuffle, seed=100)
